...
sqlLines.push("-- VENDOR-SAFE: only demo rows (vendor_id is null) are replaced; real");
sqlLines.push("-- vendor-uploaded products and existing categories are never touched.");
sqlLines.push("begin;");
sqlLines.push("delete from public.product_images where product_id in (select id from public.products where vendor_id is null);");
sqlLines.push("delete from public.products where vendor_id is null;");
...
for (const category of CATEGORIES) {
  sqlLines.push(
    `insert into public.categories (slug, name, icon, hero_tagline, hero_image) values ('${sqlEscape(category.slug)}', '${sqlEscape(
      ...
      `on conflict (slug) do update set name = excluded.name, icon = excluded.icon, hero_tagline = excluded.hero_tagline, hero_image = excluded.hero_image;`,
  );
}
...
for (const p of products) {
  sqlLines.push(
    `insert into public.products (slug, name, brand, category_id, price, compare_at_price, condition, is_featured, badge, stock_quantity, rating, review_count, description, specs, status) values (` +
      ...
      }, ${p.stock_quantity}, ${p.rating}, ${p.review_count}, '${sqlEscape(p.description)}', ${sqlJson(p.specs)}, 'published') on conflict (slug) do nothing;`,
  );
  p.images.forEach((img, i) => {
    // Scoped to vendor_id-is-null so a slug collision can never attach demo
    // images to a real vendor listing; skips cleanly if the insert was skipped.
    sqlLines.push(
      `insert into public.product_images (product_id, url, alt_text, sort_order) ` +
        `select id, '${sqlEscape(img.url)}', '${sqlEscape(img.alt)}', ${i} from public.products where slug = '${sqlEscape(
          p.slug,
